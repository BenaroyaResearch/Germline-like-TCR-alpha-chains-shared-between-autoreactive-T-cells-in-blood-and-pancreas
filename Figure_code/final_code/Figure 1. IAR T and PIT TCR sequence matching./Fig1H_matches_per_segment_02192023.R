...
##########################
## Fig1H. check matches throughout TRA segment, all HLA
...
toCalc = subset(toCalc, aJunc1 %in% all$junction) # 2345

toCalc$pair = paste0(toCalc$aJunc1, toCalc$aJunc2)
toCalc = toCalc[!duplicated(toCalc$pair),] # 2099
length(unique(toCalc$aJunc1) # 942
length(unique(toCalc$aJunc2)) # 1187
...
dfV = data.frame(table(toCalc$vMatch, toCalc$lv)) # sum(dfV$Freq) = 2078
dfV$segment = c("Vgene")
...
quartz(width=12,height=6, dpi=72)  ### open plotting window
...
filename = paste0("Fig1H_", "TRA_segment_by_mismatches_combined_cohorts", ".pdf") 
ggsave(filename, p)
...
#2     TRUE 0 mismatches    18   Vgene   87 0.2068966
#4     TRUE   1 mismatch   330   Vgene 2237 0.1475190
