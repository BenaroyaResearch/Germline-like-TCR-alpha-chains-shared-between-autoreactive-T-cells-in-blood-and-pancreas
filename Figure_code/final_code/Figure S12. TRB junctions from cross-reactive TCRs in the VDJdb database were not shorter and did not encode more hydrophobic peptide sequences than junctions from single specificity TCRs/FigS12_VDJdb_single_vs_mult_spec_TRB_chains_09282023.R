...
filename = paste0("FigS8A_", vdjdbVar, "_chain_density_number_of_specificities_in_VDJdb.pdf")
ggsave(filename, p)
...
noEpiPerJunc =  ddply(vdjdb,.(gene, junction, vdjdb.score, juncLen), plyr::summarize, sum = length(antigen.gene)) # 20830
...
p = last_plot()
setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs/")
filename = paste0("FigS8B_", vdjdbVar, "_CDR3_length_by_number_of_specificities_in_VDJdb.pdf")
ggsave(filename, p)
...
summary(toPlotSub1$CDR3) # median = 39
summary(toPlotSub2$CDR3) # median = 39

summary(toPlotSub1$Nregion) # median = 5
summary(toPlotSub2$Nregion) # median = 5

ks.test(toPlotSub1$CDR3, toPlotSub2$CDR3) # p-value 0.5969, or NS
ks.test(toPlotSub1$Nregion, toPlotSub2$Nregion) # p-value 0.9474, or NS
ks.test(toPlotSub1$hydro, toPlotSub2$hydro) # p-value 0.9674, or NS
...
ggplot(toPlot, aes(x = Nregion)) + geom_density(aes(fill = NoEpitopes), alpha = 0.5, adjust = 0.3) # y= after_stat(scaled)
...
p = last_plot()
setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs/")
filename = paste0("FigS8C_", vdjdbVar, "_Nregion_length_by_number_of_specificities_in_VDJdb.pdf")
ggsave(filename, p)
...
p = last_plot()
setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs/")
filename = paste0("FigS8D_", vdjdbVar, "_hydrophobicity_density_by Nos_specificities_in_VDJdb.pdf")
ggsave(filename, p)
