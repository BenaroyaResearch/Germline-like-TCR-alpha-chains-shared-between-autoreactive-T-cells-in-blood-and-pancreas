...
all = subset(tcrsCombSub3, set == "IAR2") # 2725 IAR1, 2295 IAR2
all.u = all[!duplicated(all$junction),] # 2174 IAR1, 2179 IAR2
...
var2 = mnTcrs.u$junction %in% intersect(mnTcrs$junction, all.u$junction)
...
filename = "Fig_S1A_barplot_Perfectly matched IAR T cell TCR junctions in PIT TCRs_cohort2.pdf"
ggsave(filename, p)

##################
## make plot TRA/TRA overlap in PIT
...
xlab = "PIT TCR chain\n"
ylab = "Number perfectly matched IAR junctions"
last_plot() + labs(x = xlab, y = ylab)
#last_plot() + coord_flip()
last_plot() + scale_y_continuous(breaks = c(0,20,40,60,80, 100))
p = last_plot()

setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs")
filename = "Fig_S1B_barplot_Perfectly matched PIT TCR junctions in IAR TCRs_cohort2.pdf"
ggsave(filename, p)

##################
## make plot CD4/CD8 overlap in PIT
...
xlab = "PIT T cell subset\n"
ylab = "Number perfectly matched IAR junctions"
last_plot() + labs(x = xlab, y = ylab)
#last_plot() + coord_flip()
last_plot() + scale_y_continuous(breaks = c(0,20,40,60))
p = last_plot()

setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs")
filename = "Fig_S1C_barplot_Perfectly matched IAR TCR junctions in PIT CD4 CD8 cells_cohort2.pdf"
ggsave(filename, p)
