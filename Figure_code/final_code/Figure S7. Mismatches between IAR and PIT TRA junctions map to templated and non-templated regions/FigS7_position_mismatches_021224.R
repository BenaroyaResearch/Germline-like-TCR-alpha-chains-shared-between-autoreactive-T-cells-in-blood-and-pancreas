...
#toPlotSub = data.frame(aJunc1 = toPlot$aJunc1, aJunc2 = toPlot$aJunc2, mismatch = toPlot$random, vEnd = toPlot$vEndAA, jStart = toPlot$jStartAA) # use random subset 
toPlotSub$btwn = toPlot$observed >= toPlot$vEndAA & toPlot$observed < toPlot$jStartAA # 74%
toPlotSub$btwn = toPlot$observed > toPlot$vEndAA & toPlot$observed < toPlot$jStartAA # 41%
toPlotSub$btwn = toPlot$observed == toPlot$vEndAA & toPlot$observed < toPlot$jStartAA # 41%
...
temp1 = data.frame(value = as.numeric(table(toPlot$observed)), set = c("mismatch"))
temp2 = data.frame(value = as.numeric(table(toPlot$random)), set = c("random"))
...
ggplot(temp3, aes(x = value)) + geom_density(aes(fill = set), alpha = 0.5) + facet_wrap(~set)
...
ks.test(var1$value, var2$value, alternative = c("greater")) # < 2.2e-16
ks.test(var1$value, var3$value, alternative = c("greater")) # < 2.2e-16
ks.test(var1$value, var2$value, alternative = c("less")) # 1
