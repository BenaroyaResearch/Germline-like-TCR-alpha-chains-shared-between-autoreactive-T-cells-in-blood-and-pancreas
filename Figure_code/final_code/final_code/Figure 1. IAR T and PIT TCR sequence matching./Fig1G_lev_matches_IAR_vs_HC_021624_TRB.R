...
all = subset(tcrsCombSub3.u, set == "IAR1" & chainType == "TRB") # 
all.u = all
...
lev = read.csv("Levenshtein_index_TRB_chains_from_IAR_CD4_with_0_1_mm.csv", stringsAsFactors = F)
lev = subset(lev, lv<9)
...
filename = "Fig_1G_IAR T cell and HC TRA junctions comparisons over a range_of_LI_values.pdf"
ggsave(filename, p)
...
#1Res.Df      RSS Df Sum of Sq      F Pr(>F)
#1      7 60680041                           
#2      8 87780425 -1 -27100384 3.1263 0.1204
#---
...
confint(r.x)
#                   2.5 %       97.5 %
#(Intercept) -883.5956668 4877.450441
#sum.y          0.9956369    1.030229
...
## calculate fraction of IAR matches
...
mnTrb.u = subset(mnTcrs.u, chainType == "TRB")	# 5143	

fracLevTra = 1309/4614 # 28.4%
