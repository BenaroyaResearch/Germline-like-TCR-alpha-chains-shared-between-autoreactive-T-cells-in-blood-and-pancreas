...
toPlot = tcrsCombSub3Tra

## modify data

toPlot$studyGroup = gsub("AAbNeg", "HC", toPlot$studyGroup)
toPlot$studyGroup = gsub("1AAb", "AAb+", toPlot$studyGroup)
toPlot$studyGroup = gsub("2AAb", "AAb+", toPlot$studyGroup)
...
toPlot1 = subset(toPlot, expanded == "E") # 1000 or 26%
toPlot2 = subset(toPlot, expanded == "NE") # 2907 or 74%
...
## consolidate Aab+ groups

toPlot1$studyGroup = gsub("1AAb", "AAb+", toPlot1$studyGroup )
toPlot1$studyGroup = gsub("2AAb", "AAb+", toPlot1$studyGroup )
toPlot2$studyGroup = gsub("1AAb", "AAb+", toPlot2$studyGroup )
toPlot2$studyGroup = gsub("2AAb", "AAb+", toPlot2$studyGroup )

df1 = ddply(toPlot1,.(studyGroup, PITmatch), plyr::summarize, no. = length(junction))#
df1$studyGroup = factor(df1$studyGroup, levels = c("HC", "AAb+", "newT1D", "T1D"))
...
df2$studyGroup = factor(df2$studyGroup, levels = c("HC", "AAb+", "newT1D", "T1D"))

sizeExpand2 = ddply(df2,.(studyGroup), plyr::summarize, sum = sum(no.)) # same as original df2
df2$tot = sizeExpand2$sum[match(df2$studyGroup, sizeExpand2$studyGroup)]
df2$freq = df2$no./df2$tot
df2$E = c("non-Expanded")

df3 = rbind(df1, df2)# 3907

temp1 = subset(df3, PITmatch == "non-PIT-matched" & studyGroup == "HC");sum(temp1$tot) # 575
temp2 = subset(df3, PITmatch == "non-PIT-matched" & studyGroup == "AAb+");sum(temp2$tot) # 632 
temp3= subset(df3, PITmatch == "non-PIT-matched" & studyGroup == "newT1D");sum(temp3$tot) # 1859
temp4 = subset(df3, PITmatch == "non-PIT-matched" & studyGroup == "T1D");sum(temp4$tot) # 841

temp5 = subset(df3, PITmatch == "PIT-matched" & studyGroup == "HC");sum(temp5$tot)
temp6 = subset(df3, PITmatch == "PIT-matched" & studyGroup == "AAb+");sum(temp6$tot)
temp7= subset(df3, PITmatch == "PIT-matched" & studyGroup == "newT1D");sum(temp7$tot)
temp8 = subset(df3, PITmatch == "PIT-matched" & studyGroup == "T1D");sum(temp8$tot)
...
ggplot(df3, aes(x = studyGroup, y = freq)) + geom_col(position = "stack", aes(fill = PITmatch), alpha = 01) 
...
last_plot() + scale_y_continuous(limits = c(0, 1.40), breaks = c(0, 0.25, 0.5, 0.75, 1.0))
last_plot() + theme(axis.text.x=element_text(angle=45, hjust=1))

p = last_plot()

setwd("/Users/peterlinsley/Desktop/PIT_TCR_paper_code/Figure_PDFs/")
filename = "Fig1K_frxn_PIT_matches_by_jdisease_group_facet_expanded_cells.pdf"
...
var1T = subset(df3, studyGroup == "HC")
var2T = subset(df3, studyGroup == "AAb+")
var3T = subset(df3, studyGroup == "newT1D")
var4T = subset(df3, studyGroup == "T1D")
...
test1 = fisher.test(rbind(a,b)); test1$p.value # 0.02055067
test2 = fisher.test(rbind(a,c)); test2$p.value # 0.02624178
test3 = fisher.test(rbind(a,d)); test3$p.value # 0.6973611
test5 = fisher.test(rbind(b,c)); test5$p.value # 0.3383416
test6 = fisher.test(rbind(b,d)); test6$p.value # 0.02554242
test8 = fisher.test(rbind(c,d)); test8$p.value # 0.02274046

## adjust by test p-values for muiltiple testing

result = c(test1$p.value, test2$p.value, test3$p.value, test5$p.value, test6$p.value, test8$p.value)
p.adjust(result)
# 1] 0.1233040 0.1233040 0.6973611 0.6766832 0.1233040 0.1233040

# none are significant after multiple testing

## tests

df3 = df2 # non-expanded
...
test1 = fisher.test(rbind(a,b)); test1$p.value # 0.1615882
test2 = fisher.test(rbind(a,c)); test2$p.value # 9.725188e-09
test3 = fisher.test(rbind(a,d)); test3$p.value # 0.2481896
test5 = fisher.test(rbind(b,c)); test5$p.value # 0.000146862
test6 = fisher.test(rbind(b,d)); test6$p.value # 0.6923682
test8 = fisher.test(rbind(c,d)); test8$p.value # 1.091219e-07
...
p.adjust(result)
#[1] 5.625423e-02 2.716332e-10 2.877175e-01 1.394160e-04 3.526992e-01 1.200370e-06
