...
toPlot3$PITmatch = gsub("TRUE", "PIT-matched", toPlot3$PITmatch)
toPlot3$PITmatch = gsub("FALSE", "non-PIT-matched", toPlot3$PITmatch)


if(dev.cur()>1)dev.off()

quartz(width=10,height=9, dpi=72)  ### open plotting window
...
ggplot(toPlot3, aes(x = traLen, y = (traR13))) + geom_jitter(aes(shape = PITmatch, stroke = 2))
...
#last_plot() + scale_color_manual(values = pal, name = "PIT match")# custom colors
...
last_plot() + theme(legend.position = c(0.7, 0.8))
...
## Stats using linear model

toModel = toPlot3

m1 = lm((traR13) ~ traLen , toModel); summary(m1) 
summary(m1)
#Coefficients:
#            Estimate Std. Error t value Pr(>|t|)    
#(Intercept)  3.20616    0.60190   5.327 1.14e-05 ***
#traLen      -0.18199    0.04266  -4.266 0.000205 ***

### KS test 

var1 = subset(toPlot3, PITmatch == "PIT-matched")
var2 = subset(toPlot3, PITmatch == "non-PIT-matched")

ks.test(var1$contacts, var2$contacts) #p-value = 0.2228
